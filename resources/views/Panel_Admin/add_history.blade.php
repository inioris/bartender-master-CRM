...
@foreach($datos as $data)
    <tr>
        <td>{!! $data->title !!}</td>
        <td><img width="150px" src="{{ url($data->imagen) }}"></td>
        <td><a href="/history/{{$data->id}}/edit" onclick="#editar" class="btn modal-trigger btn-min waves-effect waves-light blue">Editar</a></td>
...
<!-- Modal Structure -->
  <div id="modal1" class="modal z-depth-4">
    <div class="modal-content">
      <div class="row">
    <form class="col s12" method="POST" action="/history" enctype="multipart/form-data" id="recetas-form">
        {{ csrf_field() }}
      <div class="row">
        <div class="input-field col s6">
          <input placeholder="Title" name="title" maxlength="100" type="text" class="validate" required>
          <label>Title</label>
        </div>
      <div class="row">
       ...
       <button class="btn waves-effect waves-light" type="submit" id="Guardar">Guardar
        ...
        <a href="#!" class="modal-close btn waves-effect waves-light right">Cancelar</a>
    </form>
  </div>
    </div>
  </div>
        ...
        <div class="row">
            ...
            <div class="col s12 m12 modal-header">
                <div class="nav-wrapper">
                  <div class="col s12">
                    <a class="breadcrumb">Eliminaras:@if(isset($data->title)) {{$data->title}} @endif</a>
                  </div>
                </div>
            </div>
